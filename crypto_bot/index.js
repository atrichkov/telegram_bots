...
bot.command('start', ctx => {
    ...
    setInterval(async () => {
        ...
        const percentageChange = (((lastPrice - rate.usd) / lastPrice) * 100).toFixed(2)
        
        if (lastPrice && Math.abs(percentageChange) > 10)
            bot.telegram.sendMessage(chatId, `bitcoin price is changed by ${percentageChange}% to ${rate.usd}`)
